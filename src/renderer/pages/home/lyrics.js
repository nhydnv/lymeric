...
export const displayLyrics = async () => {  
  ...
  if (state && !state['device']['is_private_session']) {
    ...
    if (currentTrack.lyrics) {
      ...
      setLyricsDisplay(
        // Set previous lyric, blank if none
        lineIndex > 0 ? currentTrack.lyrics['lyrics']['lines'][lineIndex - 1]['words'] : '',
        // Set current lyric, music note if none
        lineIndex !== -1 ? currentTrack.lyrics['lyrics']['lines'][lineIndex]['words'] : `\u{266A}`,
         // Set next lyric, blank if none
        lineIndex < currentTrack.lyrics['lyrics']['lines'].length - 1 ? 
                  currentTrack.lyrics['lyrics']['lines'][lineIndex + 1]['words'] : ``,
      );
      clampLyricHeight();
    } else { 
      // No lyrics available
      setLyricsDisplay('No lyrics available for this track.', '\u{266A}', '');
    }
  } else {
    disablePlayback();
    ...
    setLyricsDisplay(
      '',
      state ? 'Disable private listening and relaunch the app to see lyrics.' : '\u{266A} Start playing something \u{266A}',
      ''
    );
  }
};

export const setLyricsDisplay = (prev, main, next) => {
  lyricPrev.textContent = prev;
  lyricMain.textContent = main;
